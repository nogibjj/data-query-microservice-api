...
def get_country_plot(country):
    ...
    cursor.execute(
        f"SELECT * FROM import.globaltemperaturesbycountry where country = '{country}' ORDER BY dt ASC;"
    )
    ...
    df_country.drop_duplicates(subset=["year"], keep="first").plot(
        x="year",
        y="temperature",
        kind="line",
...
def get_country_year_temp(country, year):
    cursor.execute(
        f"SELECT MAX(averagetemperature), MIN(averagetemperature), MEAN(averagetemperature) FROM import.globaltemperaturesbycountry where country = '{country}' and dt like '%{year}%';"
    )
    country_data = cursor.fetchall()
