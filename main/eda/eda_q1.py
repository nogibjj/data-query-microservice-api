...
def get_country_plot(country):
    ...
    cursor.execute(f"SELECT * FROM import.globaltemperaturesbycountry where country = '{country}';")
    country_data = cursor.fetchall()
    df_country = pd.DataFrame(country_data, columns=[desc[0] for desc in cursor.description])
...
def get_country_year_temp(country, year):
    cursor.execute(f"SELECT * FROM import.globaltemperaturesbycountry where country = '{country}' and dt like '%{year}%';")
    country_data = cursor.fetchall()
    df_country = pd.DataFrame(country_data, columns=[desc[0] for desc in cursor.description])
    df_country = df_country.sort_values(by=['dt'], ascending=True)
    df_country = df_country.replace(to_replace='', value = np.nan,regex=True)
...
def get_city_year_temp(country, year):
    cursor.execute(f"SELECT * FROM import.globaltemperaturesbymajorcity where country = '{country}' and dt like '%{year}%';")
    country_data = cursor.fetchall()
    df_country = pd.DataFrame(country_data, columns=[desc[0] for desc in cursor.description])
    df_country = df_country.sort_values(by=['dt'], ascending=True)
    df_country = df_country.replace(to_replace='', value = np.nan,regex=True)
    ...
    df_country['year'] = df_country['dt'].str[:4].astype(int)
    df_country['averagetemperature'] = df_country['averagetemperature'].astype(float)
    df_country['temperature'] = df_country.groupby(['year', 'city'])['averagetemperature'].transform('mean')
